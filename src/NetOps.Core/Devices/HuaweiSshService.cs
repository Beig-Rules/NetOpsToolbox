...
/// <summary>
/// Huawei VRP (AR / NE / S-series style) over SSH — read-only.
/// Disables paging with screen-length 0 temporary.
/// </summary>
public sealed class HuaweiSshService
{
    public Task<DeviceBackupResult> DisplayVersionAsync(
        string host, string username, string password,
        int port = 22, CancellationToken ct = default)
        => RunAsync(host, username, password, port, "display version", null, ct);

    public Task<DeviceBackupResult> DisplayCurrentConfigurationAsync(
        string host, string username, string password, string localDirectory,
        int port = 22, CancellationToken ct = default)
        => RunAsync(host, username, password, port, "display current-configuration", localDirectory, ct);
        ...
        try
        {
            return await Task.Run(() =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(localDirectory))
                {
                    ...
                    path = Path.Combine(localDirectory, $"huawei_{safe}_{DateTime.UtcNow:yyyyMMdd_HHmmss}.txt");
                ...
                {
                    Success = true,
                    Message = path is null ? "Huawei VRP command OK" : "Saved: " + path,
                    LocalPath = path,
                    Preview = preview
                };
