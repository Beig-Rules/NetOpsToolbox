...
public partial class MainWindow : Window
{
    ...
    private void FillModels()
    {
        ...
        foreach (var m in brand.Models) ModelBox.Items.Add(m.Model);
        if (ModelBox.Items.Count > 0) ModelBox.SelectedIndex = 0;
    }

    private void Nav_Click(object sender, RoutedEventArgs e)
    {
        var tag = (sender as FrameworkElement)?.Tag as string ?? "Dashboard";
        ...
        if (tag == "Settings")
            ContentOther.Text =
                "NetOps Toolbox · Proprietary · Beig-Rules\n" +
    ...
    {
        try { RefreshJobsAndSchedule(); return; } catch { }
    ...
    private (string host, string user, string pass, int port, string? enable) SshCreds()
    {
        ...
        return (host, user, pass, port, enable);
    }

    private async void MikrotikTest_Click(object s, RoutedEventArgs e)
    {
        var (host, user, pass, port, _) = SshCreds();
        DeviceSshOutput.Text = "MT test…";
        var r = await _mt.TestConnectionAsync(host, user, pass, port).ConfigureAwait(true);
        DeviceSshOutput.Text = FormatBackup(r);
        LogJob("MTTest", r.Success ? "OK" : "FAIL");
    }

    private async void MikrotikBackup_Click(object s, RoutedEventArgs e)
    {
        var (host, user, pass, port, _) = SshCreds();
        if (MessageBox.Show($"Export MikroTik config from {host}?", "Confirm", MessageBoxButton.OKCancel) != MessageBoxResult.OK) return;
        DeviceSshOutput.Text = "MT export…";
        var r = await _mt.ExportConfigAsync(host, user, pass, BackupDir, port).ConfigureAwait(true);
        DeviceSshOutput.Text = FormatBackup(r);
        LogJob("MTExport", r.Success ? "OK" : "FAIL");
    }

    private async void CiscoVersion_Click(object s, RoutedEventArgs e)
    {
        ...
        DeviceSshOutput.Text = FormatBackup(r);
        LogJob("CiscoVer", r.Success ? "OK" : "FAIL");
    }

    private async void CiscoShowRun_Click(object s, RoutedEventArgs e)
    {
        var (host, user, pass, port, enable) = SshCreds();
        if (MessageBox.Show($"Cisco show running-config {host}?", "Confirm", MessageBoxButton.OKCancel) != MessageBoxResult.OK) return;
        ...
        DeviceSshOutput.Text = FormatBackup(r);
        LogJob("CiscoRun", r.Success ? "OK" : "FAIL");
    }

    private async void UbntId_Click(object s, RoutedEventArgs e)
    {
        var (host, user, pass, port, _) = SshCreds();
        DeviceSshOutput.Text = "UBNT identity…";
        var r = await _ubnt.IdentityAsync(host, user, pass, port).ConfigureAwait(true);
        DeviceSshOutput.Text = FormatBackup(r);
        LogJob("UbntId", r.Success ? "OK" : "FAIL");
    }

    private async void UbntExport_Click(object s, RoutedEventArgs e)
    {
        var (host, user, pass, port, _) = SshCreds();
        if (MessageBox.Show($"UBNT export {host}?", "Confirm", MessageBoxButton.OKCancel) != MessageBoxResult.OK) return;
        DeviceSshOutput.Text = "UBNT export…";
        var r = await _ubnt.ExportConfigAsync(host, user, pass, BackupDir, port).ConfigureAwait(true);
        DeviceSshOutput.Text = FormatBackup(r);
        LogJob("UbntExport", r.Success ? "OK" : "FAIL");
    }

    private static string FormatBackup(DeviceBackupResult r)
        => (r.Success ? "OK\n" : "FAIL\n") + r.Message +
    ...
    private void QueueAll(JobKind kind, string vendorHint)
    {
        ...
        var filtered = _vault.Entries
            .Where(v => string.IsNullOrWhiteSpace(v.Vendor)
                        || v.Vendor.Contains(vendorHint, StringComparison.OrdinalIgnoreCase))
            .ToList();
        if (filtered.Count == 0)
        {
            JobsPanelText.Text = $"No vault entries for {vendorHint}. Save credentials on Devices first.";
            return;
        }
        _jobs.EnqueueFromVault(filtered, kind, _vault.UnprotectPassword, _vault.UnprotectEnablePassword);
        RefreshJobsPanel();
        LogJob("Queue", kind + " x" + filtered.Count);
    }

    private void JobsRefresh_Click(object s, RoutedEventArgs e)
    {
        try { RefreshJobsAndSchedule(); }
        catch { RefreshJobsPanel(); }
    }
    ...
    private async void RunDiagnose_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            sb.AppendLine();
            sb.AppendLine("--- Solutions (RUN = safe auto action) ---");
            sb.Append(FormatSolutionsAnnotated(report));
    ...
    {
        ToolsOutput.Text = await _tools.PingAsync(ToolsInput.Text).ConfigureAwait(true);
        LogJob("Ping", "OK");
    }

    private async void ToolDns_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = await _tools.DnsLookupAsync(ToolsInput.Text).ConfigureAwait(true);
        LogJob("DNS", "OK");
    }

    private async void ToolPort_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = await _tools.PortCheckAsync(ToolsInput.Text).ConfigureAwait(true);
        LogJob("Port", "OK");
    }

    private async void ToolTrace_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = await _tools.TracerouteAsync(ToolsInput.Text).ConfigureAwait(true);
        LogJob("Trace", "OK");
    }
    ...
    private async void ToolSpeed_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = "Speed test…";
        var r = await new HttpSpeedTestService().TestDownloadSpeedAsync().ConfigureAwait(true);
        ToolsOutput.Text = r.ToString();
        LogJob("Speed", r.Success ? "OK" : "FAIL");
    }

    private async void ToolPublicIp_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = await new PublicIpService().GetAsync().ConfigureAwait(true);
        LogJob("PublicIP", "OK");
    }

    private void ToolArp_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = new ArpTableService().GetArpTable();
        LogJob("ARP", "OK");
    }

    private void ToolRoute_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = new RouteTableService().GetRouteTable();
        LogJob("Route", "OK");
    }

    private void ToolNetstat_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = new NetstatService().GetListening();
        LogJob("Netstat", "OK");
    }

    private void ToolIfaces_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = new InterfaceListService().List();
        LogJob("Ifaces", "OK");
    }

    private void ToolHosts_Click(object s, RoutedEventArgs e)
    {
        ToolsOutput.Text = new HostsFileService().Read();
        LogJob("Hosts", "OK");
    }

    private void FirmwareDiagnose_Click(object s, RoutedEventArgs e)
    {
        var brandId = _catalog?.Brands.FirstOrDefault(b => b.Name == BrandBox.SelectedItem?.ToString())?.Id ?? "";
        FirmwareOutput.Text = _fw.Diagnose(brandId, ModelBox.SelectedItem?.ToString() ?? "",
            FwVersionBox.Text.Trim());
        LogJob("Firmware", "OK");
    }

    private async void SecurityScan_Click(object s, RoutedEventArgs e)
    {
        SecurityOutput.Text = "Scanning LAN…";
        _lastScan = await _lan.ScanAsync().ConfigureAwait(true);
        SecurityOutput.Text = string.Join("\n", _lastScan.Select(h => $"{h.Ip}\t{h.Mac}\t{h.Type}"));
        LogJob("LanScan", _lastScan.Count.ToString());
    }
    ...
    private void SecuritySave_Click(object s, RoutedEventArgs e)
    {
        ...
        SecurityOutput.Text = "Baseline saved: " + BaselinePath;
        LogJob("LanBase", "OK");
    }

    private void SecurityDiff_Click(object s, RoutedEventArgs e)
    {
