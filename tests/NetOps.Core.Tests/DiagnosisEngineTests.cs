...
public class DiagnosisEngineTests
{
    ...
    {
        var f = BaseLan();
        f.Connectivity.NameResolutionWorks = false;
        var report = new DiagnosisEngine().Evaluate(f);
    ...
    public void Captive_portal_heuristic_matches()
    {
        var f = BaseLan();
        f.Connectivity.NameResolutionWorks = false;
        f.Connectivity.PublicDnsReachable = false;
        var report = new DiagnosisEngine().Evaluate(f);
